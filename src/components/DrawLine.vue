  ...
  <div>
    <div class="container">
      <el-button @click="showPolygon(true)" :disabled="submissionFlag"
        >左键绘制线</el-button
      >
      <el-button @click="showPolygon(false)">右键结束绘制线</el-button>
    </div>
  </div>
</template>
...
import Vue from 'vue';
import DrawLineUtils from "@/assets/js/DrawLine";
Vue.prototype.DrawLineUtils = DrawLineUtils;
export default {
  name: "DrawLine",
  props: ["viewer"],
  data() {
  ...
  methods: {
    showPolygon(val) {
      if (val) {
        this.drawLineUtils = new this.DrawLineUtils(this.viewer);
        this.drawLineUtils.drawLineString(this.viewer)
        
        //将三维球定位到中国
     　　this.drawLineUtils.flyToInit();
